...
const documentController = {
  ...
  async list(req, res) {
    try {
      ...
      res
        .status(500)
        .render("errors/500", {
          message: "Erreur lors du chargement des pièces jointes.",
        });
    }
  },
  ...
  async showUploadForm(req, res) {
    try {
      ...
      const referentielType =
        type_entite === "AO" ? "TYPE_DOCUMENT_AO" : "TYPE_DOCUMENT_MARCHE";
      const typesDocument = await Referentiel.getByType(referentielType);
  ...
  async upload(req, res) {
    try {
      if (!req.file) {
        return res
          .status(400)
          .render("errors/500", { message: "Aucun fichier reçu." });
      }

      const { type_entite, entite_id, type_document_id, observation } =
        req.body;
      ...
      if (!type_entite || !entite_id || !type_document_id) {
        ...
        return res
          .status(400)
          .render("errors/500", { message: "Champs obligatoires manquants." });
      }

      const relativeFolder =
        type_entite === "AO" ? "uploads/ao" : "uploads/marches";
  ...
  async download(req, res) {
    try {
      ...
      res
        .status(500)
        .render("errors/500", { message: "Erreur lors du téléchargement." });
    }
  },
